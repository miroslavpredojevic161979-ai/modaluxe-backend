...
app.post('/webhook', express.raw({ type: '*/*' }), async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (!orderId && !sessionId) {
      console.error('Stripe webhook: nedostaje referenca na narud�bu.', { sessionId });
      return res.status(500).json({ error: 'Stripe webhook nema referencu na narud�bu.' });
    }
    ...
    try {
      ...
      if (!existingOrder) {
        console.error('Stripe webhook: narud�ba nije prona�ena.', { orderId, sessionId });
        return res.status(500).json({ error: 'Stripe webhook nije prona�ao narud�bu.' });
      }
      ...
      if (existingOrder.invoice_url) {
        ...
        if (!finalizedOrder || finalizedOrder.status !== 'PAID' || !finalizedOrder.invoice_url) {
          console.error('Stripe webhook: postoje�i invoice_url nije potvr�en uz status PAID.', { orderId: existingOrder.id, sessionId });
          return res.status(500).json({ error: 'Postoje�i invoice_url nije potvr�en uz status PAID.' });
        }
      } else {
        ...
        if (!soloRacun || !soloRacun.pdf) {
          console.error('Stripe webhook: Solo ra�un nije uspje�no generiran.', { orderId: existingOrder.id, sessionId, soloRacun });
          return res.status(500).json({ error: 'Solo ra�un nije uspje�no generiran.' });
        }
        ...
        if (!finalizedOrder || finalizedOrder.status !== 'PAID' || !finalizedOrder.invoice_url) {
          console.error('Stripe webhook: status PAID ili invoice_url nisu uspje�no spremljeni.', { orderId: existingOrder.id, sessionId });
          return res.status(500).json({ error: 'Status PAID ili invoice_url nisu uspje�no spremljeni.' });
        }
        ...
        if (!verifiedOrder || verifiedOrder.status !== 'PAID' || !verifiedOrder.invoice_url) {
          console.error('Stripe webhook: zavr�na provjera nije potvrdila PAID i invoice_url.', { orderId: existingOrder.id, sessionId });
          return res.status(500).json({ error: 'Zavr�na provjera nije potvrdila PAID i invoice_url.' });
        }

        sendPackingSlipsToSuppliers(verifiedOrder, parseJsonSafe(verifiedOrder.items, [])).catch((e) => console.error('X Gre�ka dobavlja�i Stripe:', e));
        ...
        if (verifiedOrder.email) {
          try {
            ...
            await transporter.sendMail({
              from: `"KI�FALUBA j.d.o.o." <${process.env.EMAIL_USER}>`,
              to: verifiedOrder.email,
              bcc: process.env.EMAIL_USER,
              subject: `Fiskalizirani ra�un za narud�bu br. ${invoiceNumber}`,
              html: `
                <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #D4AF37; text-align: center;">
                  <h2>Hvala na kupnji!</h2>
                  <p>Va�a uplata je uspje�no obra�ena.</p>
                  <p>Slu�beni fiskalizirani ra�un nalazi se u <strong>privitku ovog e-maila</strong>, a mo�ete ga preuzeti i klikom na gumb ispod:</p>
                  <a href="${verifiedOrder.invoice_url}" style="background-color: #D4AF37; color: black; padding: 15px 25px; text-decoration: none; font-weight: bold; border-radius: 5px; display: inline-block; margin: 20px 0;">PREUZMI PDF RA�UN</a>
                </div>
              `,
              attachments: [
                {
            ...
            console.error('Stripe webhook: ra�un je spremljen, ali slanje maila nije uspjelo.', mailErr);
          }
        }
      }
    } catch (dbErr) {
...
app.get('/orders/:id/payment-status', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Narud�ba nije prona�ena.' });
    }
