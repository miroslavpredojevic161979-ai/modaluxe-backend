...
const generateStornoPDFInvoice = (orderData, originalInvoiceNumber, stornoInvoiceNumber, filePath) => {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(16).font('Helvetica-Bold').fillColor('#e53e3e').text('KISFALUBA - STORNO', { align: 'center' });
    doc.moveDown(2);
    ...
    items.forEach((item) => {
      ...
      const qty = item.qty || item.quantity || 1;
      const price = Number(item.price).toFixed(2) + ' EUR';
      startY = drawTable1Row(startY, name, `-${qty}`, `-${price}`, false);
    });
    ...
    if (disc && disc.amount > 0) {
        doc.font('Helvetica-Bold').fontSize(9).fillColor('#22543d').text(`Storniran popust (${disc.code || 'Promo'}): +${Number(disc.amount).toFixed(2)} EUR`, 300, doc.y, { width: 250, align: 'right' });
        doc.y += 5;
    }

    doc.font('Helvetica-Bold').fontSize(10).fillColor('#e53e3e').text(`UKUPNI STORNO: -${Number(orderData.total).toFixed(2)} EUR`, 300, doc.y, { width: 250, align: 'right' });
    doc.font('Helvetica').fontSize(8).fillColor('#666666').text(fixText('Drustvo nije u sustavu PDV-a. PDV nije obracunan.'), 300, doc.y, { width: 250, align: 'right' });
    doc.fillColor('#000000');
    doc.moveDown(2);
    ...
    doc.font('Helvetica-Bold').fontSize(10).fillColor('#e53e3e').text(`Ukupan iznos storna: -${Number(orderData.total).toFixed(2)} EUR`, 300, doc.y, { width: 250, align: 'right' });
    doc.moveDown(3);
    doc.font('Helvetica').fontSize(8).fillColor('#666666');
    doc.text(fixText('Drustvo nije u sustavu poreza na dodanu vrijednost (PDV).'), { align: 'center' });
